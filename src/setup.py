...
def set_compiler_flags(compiler_flags, compiler_py_flags_nodist):
    ...
    :param compiler_py_flags_nodist:
    """
...
class PyBuildExt(build_ext):
    ...
    def update_sources_depends(self):
        ...
        for ext in self.extensions:
            ...
            if makedeps:
                ...
                ext.depends.extend(dep for dep in makedeps.split() if dep != "\\")
            ext.depends = [
        ...
        if not CYGWIN:
            ...
            for cand in dbm_order:
                if cand == "ndbm":
                    ...
                    if have_libgdbm_compat and (
                        have_gdbm_ndbm_h or have_gdbm_dash_ndbm_h
                    ):
                        if dbm_setup_debug:
    ...
    def detect_multibytecodecs(self):
        """detect_multibytecodecs."""
        # Hye-Shik Chang's CJKCodecs modules.
        self.addext(Extension("_multibytecodec", ["cjkcodecs/multibytecodec.c"]))
        for loc in ("kr", "jp", "cn", "tw", "hk", "iso2022"):
            self.addext(Extension(f"_codecs_{loc}", [f"cjkcodecs/_codecs_{loc}.c"]))
